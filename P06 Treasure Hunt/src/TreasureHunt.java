...
public class TreasureHunt extends PApplet{
	...
	/*
	 * Defines initial environment properties, loads background images
	 * fonts, loads the clues, and initializes the instance fields, as the 
	 * programs starts
	 */
	@Override
	public void setup() {
		InteractiveObject.setProcessing(this);
		this.getSurface().setTitle("Treasure Hunt"); //displays title 
		backgroundImage = loadImage("P6Distributables" + File.separator + "images" + File.separator + "background-scene.png");
		gameObjects = new ArrayList<InteractiveObject>();
		Action koalaAction = new Action("What a cute stuffed koala! It looks like a real one!");
		Action keyAction = new Action("Open sesame!");
		ClickableObject koala = new ClickableObject("koala", 350, 65, koalaAction);
		VisibleObject chest = new VisibleObject("chest", 365, 400);
		DroppableObject key = new DroppableObject("key", 70, 70, chest, keyAction);
		VisibleObject phone = new VisibleObject("phone", 700, 490);
		phone.deactivate();
		Action phoneAction = new Action(phone);
	...
	public void draw() {
		...
		for(int i = 0; i < gameObjects.size(); i++) {
			InteractiveObject obj = gameObjects.get(i);
			Action action = obj.update();
		...
		for(int i = 0; i < gameObjects.size(); i++) {
			InteractiveObject obj2 = gameObjects.get(i);
		...
		for(int i = 0; i < gameObjects.size(); i++) {
			gameObjects.get(i).update();
			if(gameObjects.get(i).update() != null) {
				 gameObjects.get(i).update().act(); //important need to check in documentation again
			}
		}*/
	}
	...
	* - name of object to activate (optional): activates this object when clicked
	...
	* - name of interactive object to activate (optional): activates this object
	...
	*/
	...
	DroppableObject newObject = new DroppableObject(name, x, y, (VisibleObject) dropTarget, new Action(message, activate));
